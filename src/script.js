 ...
 * Environment map
 */
rgbeLoader.load('./kart_club_1k.hdr', (environmentMap) =>
{
...
gui.add(uniforms.uSliceStart, "value").min(-Math.PI).max(Math.PI).step(0.001).name("Slice Start")
gui.add(uniforms.uSliceArc, "value").min(0).max(Math.PI * 2.0).step(0.001).name("Slice Arc")
...
gltfLoader.load("./gears.glb", (gltf)=>
{
    ...
    model.traverse((child)=>
    {
        if (child.isMesh)
        {
            //console.log(child.name); //NOMBRE DE LAS PARTES DEL MODELO
